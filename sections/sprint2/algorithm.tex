...
\todo{overvej om vi skal skrive expected outcome heroppe}
\begin{table}[]
\centering
\begin{tabular}{@{}llll@{}}
\toprule
...
For the example introduced in the beginning of this chapter the longest detour is set to be 25\% of Alice's total commute and Alice's route is modeled  as the line  $y = \frac{4}{7}-\frac{2 x}{7}$.
...
\end{table}
 ...
 In the results from the example in Table \ref{tbl:time} it can be seen that Bob receives a decent score, while Carol score is below zero Eve it's just in the acceptable range. 

Having the scores fro the two subalgorithms it is not time to run the results through the final algorithm Time Distance Analyser.
This result of this algortihm can be seen in Table \ref{tbl:timedist}.
...
\end{tabular}
\caption{The result for Alice's  match compatibility compared to the other users calculated by the Time Distance Analyser.}
\label{tbl:timedist}
\end{table}

The results in Table \ref{tbl:timedist} show that with the set detour limits in distance and time the only compatible match for Alice is Bob with a score of $0.59$. 
Although Carol score in distance the difference in time is unacceptable and thus is given a score of $0$.
Eve scores below zero in the distance parameter and is therefore also given a final score of $0$.

As mentioned earlier in this example the two modifiers $d_\gamma$ and $t_\gamma$ was set to $1$, these modifiers along with the acceptable detours in time and distance can be adjusted to adapt scores if the given outputs does not reflect the expected outcomes or the users actually willingness to share ride with a given comparison. 


Considering the complexity  of these three algorithms we start with the Time Distance Analyser, the complexity is only dependent on the calls on lines 2 and 3, the rest of the algorithm is input independent, if we ignore line 2 and 3.
Line 2-3 are the calls to the two sub algorithms.

In the Distance Analyser line 2 and 3 is where the whole route should be iterated through, the complexity of this sub algorithm is $O(|r|)$ where $|r|$ is the number of positions in route $r$.
