...
% gathering overview
The location is retrieved by utilizing the \texttt{GoogleApiClient}, then converted to an \textit{aSTEP}\todo{agree on aSTEP writing} location format.
...
When the client is ready, the \texttt{getCurrentGLocation()} is called, and the returned Android Location is stored in \texttt{currentGLocation}. 
...
This is performed in line 6 and 7.
...
The \texttt{convertToAstepLocation(Location gLocation)} takes an Android location format data, creates an aSTEP location instance based on the argument location, and returns the aSTEP location.
