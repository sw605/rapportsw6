...
The \gls{astep} API remains a REST API, thus meaning that all API calls are done through HTTP requests\footnote{Overview of all the HTTP-request available in \gls{astep} can be found at \url{http://www.astep.cs.aau.dk/}}.\todo{move to appendix with mortens liste?}
...
The edges method allows edges to provide the same\todo{what, claus?} benefits while disabling the security flaws.
